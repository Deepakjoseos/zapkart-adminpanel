...
const CountryList = () => {
  ...
  const getCountry= async (paginationParams = {}, filterParams) => {
    setLoading(true)
  ...
  const addProduct = () => {
    history.push(`/app/dashboards/locality/country/add-country`)
  }

  const viewDetails = (row) => {
    history.push(
      `/app/dashboards/locality/country/edit-country/${row.id}`
    )
  }

  const deleteRow = async (row) => {
    const resp = await countryService.deleteCountry(row.id)
  ...
  const handleFilterSubmit = async () => {
    ...
    form
      ...
      .then(async (values) => {
        ...
        getCountry({ pagination: resetPagination() }, sendingValues)
      })
      .catch((info) => {
  ...
  const handleClearFilter = async () => {
    ...
    setPagination(resetPagination())
    getCountry({ pagination: resetPagination() }, {})
    setFilterEnabled(false)
  }
