...
const ProductList = () => {
  ...
  const addProduct = () => {
    history.push(`/app/dashboards/catalog/manufacturer/add-manufacturer`)
  }

  const viewDetails = (row) => {
    history.push(
      `/app/dashboards/catalog/manufacturer/edit-manufacturer/${row.id}`
    )
  }

  const deleteRow = async (row) => {
    const resp = await manufacturerService.deleteManufacturer(row.id)
  ...
  const handleFilterSubmit = async () => {
    ...
    form
      ...
      .then(async (values) => {
        ...
        getManufacturers({ pagination: resetPagination() }, sendingValues)
      })
      .catch((info) => {
  ...
  const handleClearFilter = async () => {
    ...
    setPagination(resetPagination())
    getManufacturers({ pagination: resetPagination() }, {})
    setFilterEnabled(false)
  }
