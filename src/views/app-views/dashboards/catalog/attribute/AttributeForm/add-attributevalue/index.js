...
const AddAttributeValue = ({ history }) => {
  ...
  const [submitingLoading, setSubmittingLoading] = useState(false)
  ...
  const onAddAttributeValueHandler = async () => {
    if (newCreateAttrValue?.trim()?.length > 0) {
  ...
  return (
    <Card title={attributeName ? attributeName : 'Attribute Values'}>
      <Flex>
        {/* <Select
