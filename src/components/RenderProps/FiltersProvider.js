...
class FilterProvider extends React.Component {
      ...
      if (!isEmpty(a.search))
        Object.entries(a.search).forEach(([key, value]) => {
          ...
          filteredData = (filteredData.length > 0 ? filteredData : prev.data).filter(i => {
            console.log('************',i[key] ,typeof(i[key]), value)
            return startsWith(i[key].toLowerCase(), value.toLowerCase().replace(/\s+/g, ' ').trim())
          })
        })
  ...
  render() {
    ...
    console.log("columnsIn FilterPRovider",columns)
