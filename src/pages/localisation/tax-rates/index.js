...
import { Button, Popconfirm, Icon, Dropdown, notification, Table, Select } from 'antd'
...
const TaxRate = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Link
          ...
          to={`/localisation/tax-rates/edit/${record.slug}`}
        >
          {text}
        </Link>
      ),
    ...
    },
    {
      ...
      multiple: 1,
      render: (_, record) => {
        if (record.type === '%') return `${record.rate}%`
        return `₹${record.rate}`
      },
      // search: true,
    },
    ...
    },
    {
      ...
      render: (record) => (
        <span>
          <Link to={`/localisation/tax-rates/edit/${record._id}`}>
  ...
  return (
    <div>
      <Helmet title="Tax Rate List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>Tax Rate List</strong>
            <AddNew add attribute="tax rate" link="/localisation/tax-rates/add" />
          </div>
        </div>
