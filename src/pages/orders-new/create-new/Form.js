...
const CountryEditForm = ({ initialValues, onSubmit }) => {
  ...
  const [newaddressID, setNewaddressID] = useState("new")
  // const [newbillingID, setNewbillingID] = useState("new")
  const [newprescriptionID, setNewprescriptionID] = useState("new")
  ...
  const [{ response: couponList }] = useFetching('/api/backend/v1/coupen') 
  ...
  const onSubmitAddre = (val) => {
    console.log("val5555555555",val,address)
    if (val.success) {
      setNewaddressID(val.address.id)
      setModalOpen(false)
      // setValues((prev)=>({...prev,shippingId:val.address.id}))
      setAddress((prev) => [...prev, { ...val.address }])
     console.log("val*********",val,address)
    }
    console.log("val5555555555&&&&&&&&&&&&",val,address,newaddressID)
  }
  console.log("val5555555555*****************",address,"addressId",newaddressID ,prescriptions)
  ...
  const onSubmitPrescription = (val) => {
    ...
    if (val.success) {
      ...
      // console.log("p",prescriptions,val.prescriptions.filter((item,index,self)=>))
      setNewprescriptionID(val?.prescriptions)
      setModalOpen(false)
  ...
  const handleSubmit = (e) => {
    console.log("Event",e,"shipping",newaddressID,"prescriptions",newprescriptionID)
  ...
  const getCouponList=(e)=>{
    console.log("EVENT",e,couponList)
  }
  ...
  const formItems = [
    ...
    },
    {
      ...
      label: 'Coupen Code',
      type: (
        <Select placeholder="Select Coupon" onSelect={(e)=>getCouponList(e)}>
          {couponList?.data?.rows?.map((i) => (
            <Select.Option key={i.id} value={i.coupenCode}>
              {`${i.name} - ${i.coupenCode}`}
            </Select.Option>
          ))}
        </Select>
