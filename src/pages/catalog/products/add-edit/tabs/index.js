...
import { productSchemaForMerchant, productSchema } from 'utils/Schema'
...
// import SafetyAdvice from '../forms/Safety'
import ExtraAddDetails from '../forms/ExtraDetail'
import SafetyAdvice from '../forms/Safe'
...
const ExtraDetailFields = ['therapeutic_class', 'habitForming', 'safety_advice']
...
const FormIndex = (props) => {
  ...
  const {
    ...
    isSubmitting,
    // validateForm,
  } = useFormValidation(initialValues, productSchema, submitForm)
  ...
  React.useEffect(() => {
    ...
    if (_.intersection(errorKeys, ExtraDetailFields).length > 0)
      setErrorTabs((prev) => ({ ...prev, safety: true }))
    else setErrorTabs((prev) => ({ ...prev, safety: false }))
  }, [errors])
  ...
  return (
    <>
      {data && data.parentId !== null && data.parentId !== 'null' && (
        <Query
          url={`${CATALOG_API_URL.getProducts}/${data.parentId}?fields[]=name&fields[]=images`}
          loader={loader}
        >
          {({ data: parentData }) => {
            console.log('tabs indez', data.parentId, typeof data.parentId)
            // if (data)
            //   return (
      ...
      >
        ...
        <Tabs tabPosition={isMobile ? 'top' : 'left'} activeKey={activeKey} onChange={onChangeTab}>
          ...
          {/* {((values.prescriptionNeeded && values.prescriptionNeeded === 'true') ||
            values.prescriptionNeeded === true) && (
            <TabPane tab={<span>{errorTabs.safety && <span style={redStyle}>*</span>} Extra Details</span>} key={4}> */}
          {/* <div> */}
          {/* <div style={{ height: '150px',width:"50%" }}> */}
          {/* <ExtraAddDetails name="Extra Details" hideSubmit /> */}
          {/* </div> */}
          {/* // <div style={{ height: '300px' }}> */}
          {/* <SafetyAdvice name="Safety Advice" hideSubmit /> */}
          {/* </div> */}
          {/* // </div> */}
          {/* </TabPane> */}
          {/* )} */}
        ...
        >
          ...
          {data && user.userTypeId === 1 && (
            <TabPane tab={<span>Reviews</span>} key={7}>
              {/* <VariantFormTab name="variant" multipleTabs={data && true} /> */}
