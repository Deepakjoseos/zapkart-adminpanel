...
const AGeneral = ({ hideSubmit, hasTitle, formControls, isEdit, userType }) => {
  ...
  useEffect(() => {
    ...
    //   fetchPincodeServices(values.merchantId)
    // }
    ...
    fetchMedicineTypes()
    fetchOrganics()
    console.log('meritoooooooooo', setValues)
  }, [])
  ...
  const fetchPincodeServices = async (value) => {
    ...
    if (value && value !== '') {
      ...
      if (cData && cData.data) setPincode(cData.data)
    }
  }
  ...
  const generateCategoryTree = (c) => {
    ...
    const filterChildrenData=c.filter(item=>item.parent).map(item=>{
      return {
    ...
    console.log("filterDatas",filterChildrenData)
    ...
    console.log("********Ageneral",categoriesAGeneraltted)
  ...
  const formItems = [
    ...
    },
    {
      ...
      type: (
        <Select
          ...
          onChange={(a) => {
            setValues((prev) => ({ ...prev, pincode_services: [] }))
    ...
    },
    {
      ...
      type: (
        <Select
          ...
          onChange={(a) => {
            console.log('aPincode **************', a)
            // const {pincodes}= pincode.find((item)=>item._id===a)
            // console.log("pinocdes",pincodes)
            // const pin=[{
            //   type:a,
            //   ref:pincodes
            // }]
          ...
          // style={{ width: '100%' }}value={[{ type: d._id, ref: d.pincodes }]}
          // onPopupScroll={this.handlePopupScroll}
        >
          {pincode &&
            pincode.map((d) => (
    ...
    //       // mode="multiple"
    //       name="safety_advice"
    //       value={values.safety_advice}
    //       // showSearch
    //       style={widthStyle}
    //       placeholder="Select Safety Advice"
    //       // optionFilterProp="children"
    //       onChange={(e) => setValues((a) => ({ ...a, safety_advice: e }))}
    //       filterOption={(input, option) =>
    //         option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0
    //       }
    //     >
    ...
    //       mode="default"
    //       showSearch
    //       value={values.status_tag}
    //       placeholder="Select Status Tag"
    //       // notFoundContent={isFetchingProds ? <Spin size="small" /> : null}
    //       filterOption={false}
    //       // onSearch={fetchProducts}
    //       onChange={(a) => setValues((prev) => ({ ...prev, status_tag: a }))}
    //       style={{ width: '100%' }}
    //       // onPopupScroll={this.handlePopupScroll}
    //     >
...
{categories.map((value,i)=>
      ...
      <TreeNode value={value._id} title={value.name} isLeaf={!value.parent}>
        {value.parent && <TreeNode value={value.parent._id} title={value.parent.name} isLeaf={!value.parent.parent}>
        {value.parent.parent && <TreeNode value={value.parent.parent._id} title={value.parent.parent.name} isLeaf={!value.parent.parent.parent}/>}
        </TreeNode>
          }
        </TreeNode>
      ))}
