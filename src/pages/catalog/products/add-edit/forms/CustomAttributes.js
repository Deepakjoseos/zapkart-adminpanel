...
const FormD = (props) => {
  ...
  console.log('values', values.cutomAttributes)
  ...
  const onChangeAttrValues = (val, index) => {
    console.log('Changed attribute', val, index)
    setValues((a) => {
      a.cutomAttributes[index].values = val
      return { ...a }
    })
  }
  ...
  const onClickAddAttr = () => {
    if (values.cutomAttributes.length > 0) {
      var checkonAdd = values.cutomAttributes.filter((val) => val.attribute === null).length
      if (checkonAdd > 0) {
        console.log("We can't add")
      } else {
        setValues((a) => {
          const newData = {
            key: generateKey('attr'),
  ...
  const checkSelectValue = (val) => {
    console.log('Selected', val)
    if (val === null) {
      return Promise.reject('Please select attributes')
    } else {
  ...
  return (
    <>
      <Form onChange={onChange} onBlur={onBlur} onSubmit={onSubmit} labelAlign="right">
        ...
        {values.cutomAttributes && values.cutomAttributes.length > 0 ? (
          values.cutomAttributes
            ...
            .map((i, index, array) => {
              ...
              // if (array.length == 2 && array[0]?.attribute == null && array[1]?.attribute == null) {
              //   console.log('Find')
              //   return (
              ...
              //   )
              // }
              if (index == 1 && array[0].attribute == null) {
                console.log("We can't add")
              } else {
                return (
                  <>
                    <Row key={i.key} gutter={10}>
                      <Col span={8}>
                        <Form.Item
                          ...
                          validateStatus={errors[`cutomAttributes[${index}].attribute`] && 'error'}
                          help={errors[`cutomAttributes[${index}].attribute`]}
                          rules={[{ validator: checkSelectValue }]}
                        >
                          <Select
                            // disabled={mandatorycutomAttributes.includes(i.attribute)}
                            value={i.attribute}
                      ...
                      <Col span={12} style={{ border: '1px solid grey', borderRadius: 3 }}>
                        ...
                        >
                          <Checkbox.Group
                            ...
                            defaultValue={i.values?.map((w) => w)}
                            // defaultValue={['5fd35948d1eeab596047f594']}
        ...
        {values.attributes && values.cutomAttributes.length > 0 && !hideSubmit && (
          <Button disabled={isSubmitting} type="primary" htmlType="submit">
            Submit
          </Button>
        )}
