...
const FormD = (props) => {
  ...
  const onChangeAttrGrp = (val, index) => {
    ...
    setValues((m) => {
      m.groupPrice[index] = {}
      m.groupPrice[index].customerGroup = val
      m.groupPrice[index].price = null
      return { ...m }
    })
    // if (aVals.length > 0)
    // setAttrValues((a) => {
  ...
  const onClickAddAttr = () => {
    setValues((a) => {
      ...
      a.groupPrice = [...a.groupPrice, newData]
      return { ...a }
    })
  }
  ...
  const handleDeleteAttr = (ind) => {
    ...
    setValues((a) => {
      const filtered = a.groupPrice.filter((i, index) => index !== ind)
      console.log('updated', filtered, filtered.length)
      return { ...a, groupPrice: filtered }
    })
  }

  // const setAttrValOptions = attributeId => {
  //   const filtered = groupPriceData.filter(i => i._id === attributeId)
  ...
  return (
    <>
      <Form onChange={onChange} onBlur={onBlur} onSubmit={onSubmit} labelAlign="right">
        ...
        {values.groupPrice && values.groupPrice.length > 0 ? (
          values.groupPrice.filter(i=>i!==null).map((i, index) => {
            console.log('ffffffffffffff', values.groupPrice.length)
            return (
              <>
                <Row key={i.key} gutter={10}>
                  <Col span={10}>
                    <Form.Item
                      key={i.key}
                      validateStatus={errors[`groupPrice[${index}].customerGroup`] && 'error'}
                      help={errors[`groupPrice[${index}].customerGroup`]}
                    >
                      <Select
                        // disabled={mandatorygroupPrice.includes(i.attribute)}
                        value={i.customerGroup}
                  ...
                  <Col span={12}>
                    <Form.Item
                      key={i.key}
                      validateStatus={errors[`groupPrice[${index}].price`] && 'error'}
                      help={errors[`groupPrice[${index}].value`]}
                    >
                      {
                        <InputNumber
          ...
          <>
            <Row>
              <Form.Item
                className={`${errors.groupPrice ? 'has-error' : ''}`}
                validateStatus={errors.groupPrice}
                help={errors.groupPrice}
              />
              {!errors.groupPrice && <span>Add some groupPrice...</span>}
            </Row>
          </>
        )}

        {values.customerGroup && values.groupPrice.length > 0 && !hideSubmit && (
