...
const Products = (props) => {
  ...
  const handleMenuClick = async (e) => {
    ...
    if (state.statusClickedId) {
      const res = await editStatus(state.statusClickedId, e.key)
      console.log('isUpdated', res, 'e.key', e.key)
      if (res?.success) {
        dispatch({
  ...
  const handleMenuClick2 = async (e) => {
    ...
    if (state.statusClickedId) {
      const res = await callApi(
        `/api/catalog/v1/products/${state.statusClickedId}?adminapprovestatus=${e.key}`,
        { method: 'PATCH' },
      )
      console.log('isUpdated', res, 'e.key', e.key)
      if (res?.success) {
        dispatch({
          type: 'updateClickedProdApproveStatus',
          payload: e.key,
        })
      }
      dispatch({
  ...
  const menu2 = <Menu items={menuApproveItems} onClick={handleMenuClick2} />

  useEffect(() => {
    handleCategorySearch()
  }, [])
  console.log('state Categoo*************', state.categories)
  useEffect(() => {
