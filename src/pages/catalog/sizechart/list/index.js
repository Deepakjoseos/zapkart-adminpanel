...
const Products = () => {
  ...
  const [{ response, loading, error }] = useFetching(`/api/catalog/v1/sizeChart/query`)

  const handleMenuClick = async (e) => {
    // refetch
    // console.log('clicked on', e.key, clickedId)
    const isUpdated = await editData(
      `${CATALOG_API_URL.sizeChart}/${state.statusClickedId}`,
      { status: e.key },
      'json',
    )
    if (isUpdated) {
  ...
  const handleDelete = async (id) => {
    ...
    const isDeleted = await deleteData(`${CATALOG_API_URL.sizeChart}/${id}`)
    if (isDeleted) {
      dispatch({
        type: 'deleteProduct',
        payload: id,
  ...
  const columns = [
    {
      ...
      multiple: 1,
      render: (text, record) => (
        <Link className="utils__link--underlined" to={`${LINKS.editSizechart}/${record._id}`}>
          {text}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (record) => (
        <span>
          <Link to={`${LINKS.editSizechart}/${record._id}`}>
  ...
  return (
    <div>
      <Helmet title="Sizechart List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>Sizechart List</strong>
            <AddNew
              add
              // selectedRowKeys={selectedRowKeys}
              handleDelete={handleDelete}
              attribute="product"
              link="/catalog/sizecharts/add"
            />
          </div>
        </div>
