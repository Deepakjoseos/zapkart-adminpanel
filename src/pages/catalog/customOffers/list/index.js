...
const Products = () => {
  ...
  const [{ response, loading, error }] = useFetching(
    `${CATALOG_API_URL.customeoffer}/query?page=${state.current}&limit=${state.pageSize}`,
  )
  ...
  const handleMenuClick = async (e) => {
    ...
    const isUpdated = await editData(
      `${CATALOG_API_URL.customeoffer}/${state.statusClickedId}`,
      { status: e.key },
      'json',
    )
    if (isUpdated) {
  ...
  const handleDelete = async (id) => {
    ...
    const isDeleted = await deleteData(`${CATALOG_API_URL.customeoffer}/${id}`)
    if (isDeleted) {
      dispatch({
        type: 'deleteProduct',
        payload: id,
  ...
  const columns = [
    {
      ...
      multiple: 1,
      render: (text, record) => (
        <Link className="utils__link--underlined" to={`${LINKS.editcustomoffers}/${record._id}`}>
          {text}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (record) => (
        <span>
          <Link to={`${LINKS.editcustomoffers}/${record._id}`}>
  ...
  return (
    <div>
      <Helmet title="Custom Offers List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>Custom Offers List</strong>
            <AddNew add attribute="Custom Offer" link={`${LINKS.customoffers}/create`} />
          </div>
        </div>
