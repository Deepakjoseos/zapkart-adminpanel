...
const Products = () => {
  ...
  const [{ response, loading, error }] = useFetching(
    `${CATALOG_API_URL.saltComposition}/query?page=${state.current}&limit=${state.pageSize}`,
  )
  ...
  const handleMenuClick = async (e) => {
    ...
    const isUpdated = await editData(
      `${CATALOG_API_URL.saltComposition}/${state.statusClickedId}`,
      { status: e.key },
      'json',
    )
    if (isUpdated) {
  ...
  const handleDelete = async (id) => {
    ...
    const isDeleted = await deleteData(`${CATALOG_API_URL.saltComposition}/${id}`)
    if (isDeleted) {
      dispatch({
        type: 'deleteProduct',
        payload: id,
  ...
  return (
    <div>
      <Helmet title="Salt Composition List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>Salt Composition List</strong>
            <AddNew add attribute="Salt composition" link={`${LINKS.saltcomposition}/create`} />
          </div>
        </div>
