...
const Products = () => {
  ...
  const [{ response, loading, error }] = useFetching(
    `${CATALOG_API_URL.saltsynonims}/query?page=${state.current}&limit=${state.pageSize}`,
  )
  ...
  const handleMenuClick = async (e) => {
    ...
    const isUpdated = await editData(
      `${CATALOG_API_URL.saltsynonims}/${state.statusClickedId}`,
      { status: e.key },
      'json',
    )
    if (isUpdated) {
  ...
  const handleDelete = async (id) => {
    ...
    const isDeleted = await deleteData(`${CATALOG_API_URL.saltsynonims}/${id}`)
    if (isDeleted) {
      dispatch({
        type: 'deleteProduct',
        payload: id,
  ...
  const columns = [
    {
      ...
      multiple: 1,
      render: (text, record) => (
        <Link className="utils__link--underlined" to={`${LINKS.editsaltsynonyms}/${record._id}`}>
          {text}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (record) => (
        <span>
          <Link to={`${LINKS.editsaltsynonyms}/${record._id}`}>
  ...
  return (
    <div>
      <Helmet title="salt Synonyms List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>salt Synonyms List</strong>
            <AddNew add attribute="salt Synonyms" link={`${LINKS.saltsynonyms}/create`} />
          </div>
        </div>
