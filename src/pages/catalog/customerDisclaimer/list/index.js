...
const Products = () => {
  ...
  const [{ response, loading, error }] = useFetching(
    `${CATALOG_API_URL.disclaimer}/query?page=${state.current}&limit=${state.pageSize}`,
  )
  ...
  const handleMenuClick = async (e) => {
    ...
    const isUpdated = await editData(
      `${CATALOG_API_URL.disclaimer}/${state.statusClickedId}`,
      { status: e.key },
      'json',
    )
    if (isUpdated) {
  ...
  const handleDelete = async (id) => {
    ...
    const isDeleted = await deleteData(`${CATALOG_API_URL.disclaimer}/${id}`)
    if (isDeleted) {
      dispatch({
        type: 'deleteProduct',
        payload: id,
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <Link
          ...
          to={`${LINKS.editcustomerdisclaimer}/${record._id}`}
        >
          {text}
        </Link>
      ),
    ...
    },
    {
      ...
      render: (record) => (
        <span>
          <Link to={`${LINKS.editcustomerdisclaimer}/${record._id}`}>
  ...
  return (
    <div>
      <Helmet title="Customer Disclaimer List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>Customer Disclaimer List</strong>
            <AddNew
              add
              attribute="Customer Disclaimer"
              link={`${LINKS.customerdisclaimer}/create`}
            />
          </div>
        </div>
