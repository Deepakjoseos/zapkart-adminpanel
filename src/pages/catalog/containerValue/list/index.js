...
const Products = () => {
  ...
  const [{ response, loading, error }] = useFetching(
    `${CATALOG_API_URL.containervalue}/query?page=${state.current}&limit=${state.pageSize}`,
  )
  ...
  const handleMenuClick = async (e) => {
    ...
    const isUpdated = await editData(
      `${CATALOG_API_URL.containervalue}/${state.statusClickedId}`,
      { status: e.key },
      'json',
    )
    if (isUpdated) {
  ...
  const handleDelete = async (id) => {
    ...
    const isDeleted = await deleteData(`${CATALOG_API_URL.containervalue}/${id}`)
    if (isDeleted) {
      dispatch({
        type: 'deleteProduct',
        payload: id,
  ...
  const columns = [
    {
      ...
      multiple: 1,
      render: (text, record) => (
        <Link className="utils__link--underlined" to={`${LINKS.editcontainers}/${record._id}`}>
          {text}
        </Link>
      ),
    },
    ...
    },
    {
      ...
      render: (record) => (
        <span>
          <Link to={`${LINKS.editcontainers}/${record._id}`}>
  ...
  return (
    <div>
      <Helmet title="Containers List" />
      <div className="card">
        <div className="card-header">
          <div className="utils__title">
            <strong>Container List</strong>
            <AddNew add attribute="Container" link={`${LINKS.containers}/create`} />
          </div>
        </div>
